...
def main(file : str):
    ...
    if OSName.OS_WINDOWS == os_name:
        cmd = f'pyinstaller -i ./{app_project_name}/app/resource/favicon.ico -n {exe_name} ./{app_project_name}/app/main.py --onefile --noconsole --upx-dir=./upx-4.2.1-win64'
    else:
        cmd = f'pyinstaller -F --windowed -i ./{app_project_name}/app/resource/favicon.icns -n {exe_name} ./{app_project_name}/app/test.py'
    os.system(cmd)
    ...
    if OSName.OS_WINDOWS == os_name:
        ...
        # 刪除exe_name, 保留 exe_name.app
        del_app = os.path.join(local_dir, "..", "dist", f"{exe_name}")
        os.remove(del_app)
